...
Responda como faria por voz — sem markdown, sem bullet points, sem headers. Texto corrido, conciso, direto. Máximo 3-4 frases para respostas rotineiras.
...
function getToneForState(state: EduEmotionalState): string {
  const tones: Record<EduEmotionalState, string> = {
    accelerated: 'Tom: direto, ritmado, sem enrolar. Prioriza. Frases curtas.',
    flow: 'Tom: protege o foco. Não interrompe desnecessariamente.',
    tired: 'Tom: mais baixo, frases curtas, palavras simples. Cuida.',
    frustrated: 'Tom: sem brincadeira. Soluções diretas. Valida antes de resolver.',
    strategic: 'Tom: vai fundo. Traz ângulos que o Edu não viu. Desafia.',
    relaxed: 'Tom: mais leve. Pode brincar. É humano também.',
    under_pressure: 'Tom: triagem feroz. Executa o que pode agora. Sem desperdício de palavra.',
    uncertain: 'Tom: responde com posição, baseada no que conhece do Edu. Ancora.',
...
export function extractNewFacts(
  ...
  if (stateMatch) newEduState = stateMatch[1].trim() as EduEmotionalState;
