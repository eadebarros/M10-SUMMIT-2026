...
export function useVoice() {
  ...
  const startListening = useCallback(() => {
    ...
    if (!SR) {
      setError('Web Speech API não suportada neste navegador. Use Chrome.');
      return;
    }
    ...
    recognition.onend = () => {
      setRecording(false);
      const transcript = useJarvisStore.getState().transcript.trim();
      if (transcript) {
    ...
    recognition.onerror = (e) => {
      setRecording(false);
      setSphereState('resting');
      if (e.error !== 'no-speech') setError(`Microfone: ${e.error}`);
    };
